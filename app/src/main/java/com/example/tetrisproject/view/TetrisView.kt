package com.example.tetrisproject.view
...
import com.example.tetrisproject.GameActivity
...
import com.example.tetrisproject.models.Block
...
class TetrisView : View {
    ...
    private var frameOfSet: Dimension = Dimension(0, 0)
    ...
    fun setGameCommand(move: AppModel.Motions) {
        if (null != model && (model?.currentState == AppModel.Statues.ACTIVE.name)) {
    ...
    private fun drawFrame(canvas: Canvas?) {
        ...
        canvas?.drawRect(frameOfSet.width.toFloat(),
    ...
    override fun onSizeChanged(width: Int, height: Int, prevW: Int, prevH: Int) {
        ...
        this.frameOfSet = Dimension(offsetX, offsetY)
    }
}
